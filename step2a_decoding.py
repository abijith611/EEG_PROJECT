...
Decoding script:
  ...
  - Uses Time-binning and Temporal + Spatial Searchlight
...
def run_decoding(max_pairs=None):
    ...
    for p_idx, pair in enumerate(pairs_to_run):
        ...
        for ppt in [1, 2]:
            ...
            # FIX: Map A1-B32 channels to standard 10-20 names so MNE can find 3D coords
            ...
            for test_idx in targets_map:
                ...
                # Check if enough samples exist to perform 10-fold CV
                if len(y_valid) < 10:
                    decoding_accuracy.append(np.full(20, np.nan))
                    searchlight_acc.append(np.full((num_chan, 20), np.nan))
                    continue
                
                cv = StratifiedKFold(n_splits=10, shuffle=True, random_state=pair)
                clf = LinearDiscriminantAnalysis()
                
                acc_time = np.zeros(20)
                for t in range(20):
                    X_t = X_valid[:, :, t]
                    acc_time[t] = cross_val_score(clf, X_t, y_valid, cv=cv, n_jobs=-1).mean()
                decoding_accuracy.append(acc_time)
                
                sl_acc = np.zeros((num_chan, 20))
                
                for ch in range(num_chan):
                    # Foolproof way to get neighbors across all SciPy versions
                    row = adj_matrix[ch]
                    row_dense = row.toarray().flatten() if hasattr(row, 'toarray') else np.asarray(row).flatten()
                    neighbors = np.where(row_dense)[0]
                    
                    # Ensure the central channel is first, followed by its neighbors
                    ch_idx = [ch] + [n for n in neighbors if n != ch]
                    
                    for t in range(20):
                        X_t_sl = X_valid[:, ch_idx, t]
                        sl_acc[ch, t] = cross_val_score(clf, X_t_sl, y_valid, cv=cv).mean()
