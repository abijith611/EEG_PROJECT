...
def plot_decoding(max_pairs=None, classifier='svm'):
    ...
    for t in range(num_tests):
        ...
        for idx, (start, end) in enumerate([(0, 8), (8, 16), (16, 20)]):
            ax_main.plot(x_axis_bins[start:end], data_mean[start:end], color=colors[idx], linewidth=2)
            ax_main.fill_between(x_axis_bins[start:end], data_mean[start:end] - ci[start:end], data_mean[start:end] + ci[start:end], color=colors[idx], alpha=0.2)
            ax_main.scatter(x_axis_bins[start:end], data_mean[start:end], color=colors[idx], s=45)
        ...
        ax_main.text(4.5, 39.5, 'Decision', ha='center', va='top', fontsize=11, fontweight='bold', color='black', zorder=10)
        ax_main.text(12.5, 39.5, 'Response', ha='center', va='top', fontsize=11, fontweight='bold', color='black', zorder=10)
        ax_main.text(18.5, 39.5, 'Feedback', ha='center', va='top', fontsize=11, fontweight='bold', color='black', zorder=10)
        ...
        for t in range(num_tests):
            ...
            for d, c, lbl in zip([data_win, data_los], [color_win, color_los], ['Winners', 'Losers']):
                ...
                for start, end in [(0, 8), (8, 16), (16, 20)]:
                    ...
                    ax_main.plot(x_axis_bins[start:end], mean_d[start:end], color=c, linewidth=2, label=line_lbl)
                    ax_main.fill_between(x_axis_bins[start:end], mean_d[start:end] - ci_d[start:end], mean_d[start:end] + ci_d[start:end], color=c, alpha=0.15)
                    ax_main.scatter(x_axis_bins[start:end], mean_d[start:end], color=c, s=45)
            ...
            ax_bf.scatter(x_axis_bins, [2]*20, c=np.log10(bfs_win), cmap='RdBu_r', vmin=-6, vmax=6, s=90)
            ax_bf.scatter(x_axis_bins, [1]*20, c=np.log10(bfs_los), cmap='RdBu_r', vmin=-6, vmax=6, s=90)
            sc3 = ax_bf.scatter(x_axis_bins, [0]*20, c=np.log10(bfs_diff), cmap='RdBu_r', vmin=-6, vmax=6, s=90)
