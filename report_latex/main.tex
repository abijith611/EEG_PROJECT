...
\textbf{Our Decision:}
We deliberately deviated from the original code by using participant-specific seeds (\texttt{seed = pair * 10 + ppt}). This maintains reproducibility (same seed always gives same result) while ensuring each participant has unique pseudo-trial compositions.
...
\textbf{Note on SVM Implementation:} In our implementation, we used \texttt{SGDClassifier(loss='hinge', penalty='l2', alpha=0.0001)}, which trains a linear SVM with hinge loss and L2 regularization. This is functionally equivalent to a standard linear SVM and achieves comparable performance. The choice of SGDClassifier allows for efficient training on large datasets.
...
\taltrow Own response & Response & 729,735 & 4,178 & Both very strong, magnitude differs \\
Own response & Feedback & 16,028 & 556.17 & Both very strong, magnitude differs \\
\taltrow Opponent's response & Feedback & 87,847 & 2,520 & Both very strong, magnitude differs \\
Own previous & Decision & 8 & 6.70 & Both moderate, magnitude differs \\
\taltrow Opponent's previous & Decision & 16,659 & 5,427 & Both very strong, magnitude differs \\
\bottomrule
\end{tabular}
\end{styledtable}
\end{table}
...
\taltrow Own response (Response) & Losers & 3,337 & 65.47 & Both strong, magnitude differs \\
Own previous (Response) & Winners & No evidence & 5.18 & Moderate evidence (differs from paper) \\
\taltrow Own previous (Response) & Losers & 11 & 34.33 & Moderate-strong evidence \\
Opponent's previous (Decision) & Winners & No evidence & 21.37 & Paper: none, Ours: moderate \\
\taltrow Opponent's previous (Decision) & Losers & 2,382 & 179.89 & Strong evidence \\
\bottomrule
\end{tabular}
\end{styledtable}
\end{table}
...
\begin{itemize}
    ...
    \item Winners: No evidence (paper)
...
\begin{itemize}
    \item Own previous: Winners BF = 5.18 (moderate), Losers BF = 34.33 (strong)
    \item Opponent's previous: Winners BF = 21.37 (strong), Losers BF = 179.89 (very strong)
\end{itemize}
...
\taltrow SVM & 38.15\% (Losers) & 37.03\% (Winners) \\
Logistic & 37.91\% (Losers) & 37.85\% (Winners) \\
\taltrow Ridge & 38.06\% (Losers) & 37.47\% (Winners) \\
\bottomrule
\end{tabular}
\end{styledtable}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Classifier differences:} Despite using the same LDA algorithm, our results differ from the original, likely due to variations in underlying library implementations (\sklearn{} vs. \matlab{}'s \texttt{fitcdiscr}). We also tested LDA with and without regularization to assess sensitivity.

    \item \textbf{SVM implementation:} We found \texttt{sklearn.svm.SVC} to be prohibitively slow on our data. We used \texttt{SGDClassifier(loss='hinge')}, which is functionally equivalent to a linear SVM but trains much faster. This choice may introduce minor differences.
...
\begin{enumerate}
    \item \textbf{Neural signatures of RPS decisions exist} and can be decoded 4--5\% above the 33.33\% chance level (peak accuracy: 37.76\% for own response during Response phase)
    \item \textbf{Temporal dynamics match theory:} Own response decodable during Response and Feedback phases (BF = 556--4,178); opponent's response only decodable after visual feedback (BF = 2,520)
...
\begin{itemize}
    \item \textbf{Own previous response (Response phase):} Winners BF = 5.18 (moderate evidence), Losers BF = 34.33 (strong evidence)
    \item \textbf{Opponent's previous response (Decision phase):} Winners BF = 21.37 (moderate), Losers BF $>$ 179.89 (very strong evidence)
\end{itemize}
